...
export function navigate(screenId, params = {}) {
  ...
  // Track history only on forward navigation
  if (prev && prev !== screenId && !isGoingBack) {
    navStack.push(prev);
  }
  isGoingBack = false;
  ...
  if (nextEl) {
    ...
    if (scrollContainer) {
      ...
      try {
        ...
        console.error(`[WorkoutPAL] Render error on screen "${screenId}":`, err);
        scrollContainer.innerHTML = `
          <div style="padding:40px 24px;text-align:center;color:var(--text-secondary);">
            ...
            <div style="font-size:13px;line-height:1.6;">${err.message}</div>
          </div>`;
      }
    }
  }
